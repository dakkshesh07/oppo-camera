.class public abstract Landroid/net/IConnectivityDiagnosticsCallback$Stub;
.super Landroid/os/Binder;
.source "IConnectivityDiagnosticsCallback.java"

# interfaces
.implements Landroid/net/IConnectivityDiagnosticsCallback;


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Landroid/net/IConnectivityDiagnosticsCallback;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x409
    name = "Stub"
.end annotation

.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;
    }
.end annotation


# static fields
.field private static final blacklist DESCRIPTOR:Ljava/lang/String; = "android.net.IConnectivityDiagnosticsCallback"

.field static final blacklist TRANSACTION_onConnectivityReportAvailable:I = 0x1

.field static final blacklist TRANSACTION_onDataStallSuspected:I = 0x2

.field static final blacklist TRANSACTION_onNetworkConnectivityReported:I = 0x3


# direct methods
.method public constructor blacklist <init>()V
    .locals 1

    .line 31
    invoke-direct {p0}, Landroid/os/Binder;-><init>()V

    .line 32
    const-string v0, "android.net.IConnectivityDiagnosticsCallback"

    invoke-virtual {p0, p0, v0}, Landroid/net/IConnectivityDiagnosticsCallback$Stub;->attachInterface(Landroid/os/IInterface;Ljava/lang/String;)V

    .line 33
    return-void
.end method

.method public static blacklist asInterface(Landroid/os/IBinder;)Landroid/net/IConnectivityDiagnosticsCallback;
    .locals 2
    .param p0, "obj"    # Landroid/os/IBinder;

    .line 40
    if-nez p0, :cond_0

    .line 41
    const/4 v0, 0x0

    return-object v0

    .line 43
    :cond_0
    const-string v0, "android.net.IConnectivityDiagnosticsCallback"

    invoke-interface {p0, v0}, Landroid/os/IBinder;->queryLocalInterface(Ljava/lang/String;)Landroid/os/IInterface;

    move-result-object v0

    .line 44
    .local v0, "iin":Landroid/os/IInterface;
    if-eqz v0, :cond_1

    instance-of v1, v0, Landroid/net/IConnectivityDiagnosticsCallback;

    if-eqz v1, :cond_1

    .line 45
    move-object v1, v0

    check-cast v1, Landroid/net/IConnectivityDiagnosticsCallback;

    return-object v1

    .line 47
    :cond_1
    new-instance v1, Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;

    invoke-direct {v1, p0}, Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;-><init>(Landroid/os/IBinder;)V

    return-object v1
.end method

.method public static blacklist getDefaultImpl()Landroid/net/IConnectivityDiagnosticsCallback;
    .locals 1

    .line 239
    sget-object v0, Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;->sDefaultImpl:Landroid/net/IConnectivityDiagnosticsCallback;

    return-object v0
.end method

.method public static blacklist getDefaultTransactionName(I)Ljava/lang/String;
    .locals 1
    .param p0, "transactionCode"    # I

    .line 56
    const/4 v0, 0x1

    if-eq p0, v0, :cond_2

    const/4 v0, 0x2

    if-eq p0, v0, :cond_1

    const/4 v0, 0x3

    if-eq p0, v0, :cond_0

    .line 72
    const/4 v0, 0x0

    return-object v0

    .line 68
    :cond_0
    const-string/jumbo v0, "onNetworkConnectivityReported"

    return-object v0

    .line 64
    :cond_1
    const-string/jumbo v0, "onDataStallSuspected"

    return-object v0

    .line 60
    :cond_2
    const-string/jumbo v0, "onConnectivityReportAvailable"

    return-object v0
.end method

.method public static blacklist setDefaultImpl(Landroid/net/IConnectivityDiagnosticsCallback;)Z
    .locals 2
    .param p0, "impl"    # Landroid/net/IConnectivityDiagnosticsCallback;

    .line 229
    sget-object v0, Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;->sDefaultImpl:Landroid/net/IConnectivityDiagnosticsCallback;

    if-nez v0, :cond_1

    .line 232
    if-eqz p0, :cond_0

    .line 233
    sput-object p0, Landroid/net/IConnectivityDiagnosticsCallback$Stub$Proxy;->sDefaultImpl:Landroid/net/IConnectivityDiagnosticsCallback;

    .line 234
    const/4 v0, 0x1

    return v0

    .line 236
    :cond_0
    const/4 v0, 0x0

    return v0

    .line 230
    :cond_1
    new-instance v0, Ljava/lang/IllegalStateException;

    const-string/jumbo v1, "setDefaultImpl() called twice"

    invoke-direct {v0, v1}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    throw v0
.end method


# virtual methods
.method public whitelist test-api asBinder()Landroid/os/IBinder;
    .locals 0

    .line 51
    return-object p0
.end method

.method public blacklist getTransactionName(I)Ljava/lang/String;
    .locals 1
    .param p1, "transactionCode"    # I

    .line 79
    invoke-static {p1}, Landroid/net/IConnectivityDiagnosticsCallback$Stub;->getDefaultTransactionName(I)Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public whitelist test-api onTransact(ILandroid/os/Parcel;Landroid/os/Parcel;I)Z
    .locals 4
    .param p1, "code"    # I
    .param p2, "data"    # Landroid/os/Parcel;
    .param p3, "reply"    # Landroid/os/Parcel;
    .param p4, "flags"    # I
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Landroid/os/RemoteException;
        }
    .end annotation

    .line 83
    const-string v0, "android.net.IConnectivityDiagnosticsCallback"

    .line 84
    .local v0, "descriptor":Ljava/lang/String;
    const/4 v1, 0x1

    if-eq p1, v1, :cond_6

    const/4 v2, 0x2

    if-eq p1, v2, :cond_4

    const/4 v2, 0x3

    if-eq p1, v2, :cond_1

    const v2, 0x5f4e5446

    if-eq p1, v2, :cond_0

    .line 134
    invoke-super {p0, p1, p2, p3, p4}, Landroid/os/Binder;->onTransact(ILandroid/os/Parcel;Landroid/os/Parcel;I)Z

    move-result v1

    return v1

    .line 88
    :cond_0
    invoke-virtual {p3, v0}, Landroid/os/Parcel;->writeString(Ljava/lang/String;)V

    .line 89
    return v1

    .line 119
    :cond_1
    invoke-virtual {p2, v0}, Landroid/os/Parcel;->enforceInterface(Ljava/lang/String;)V

    .line 121
    invoke-virtual {p2}, Landroid/os/Parcel;->readInt()I

    move-result v2

    if-eqz v2, :cond_2

    .line 122
    sget-object v2, Landroid/net/Network;->CREATOR:Landroid/os/Parcelable$Creator;

    invoke-interface {v2, p2}, Landroid/os/Parcelable$Creator;->createFromParcel(Landroid/os/Parcel;)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/net/Network;

    .local v2, "_arg0":Landroid/net/Network;
    goto :goto_0

    .line 125
    .end local v2    # "_arg0":Landroid/net/Network;
    :cond_2
    const/4 v2, 0x0

    .line 128
    .restart local v2    # "_arg0":Landroid/net/Network;
    :goto_0
    invoke-virtual {p2}, Landroid/os/Parcel;->readInt()I

    move-result v3

    if-eqz v3, :cond_3

    move v3, v1

    goto :goto_1

    :cond_3
    const/4 v3, 0x0

    .line 129
    .local v3, "_arg1":Z
    :goto_1
    invoke-virtual {p0, v2, v3}, Landroid/net/IConnectivityDiagnosticsCallback$Stub;->onNetworkConnectivityReported(Landroid/net/Network;Z)V

    .line 130
    return v1

    .line 106
    .end local v2    # "_arg0":Landroid/net/Network;
    .end local v3    # "_arg1":Z
    :cond_4
    invoke-virtual {p2, v0}, Landroid/os/Parcel;->enforceInterface(Ljava/lang/String;)V

    .line 108
    invoke-virtual {p2}, Landroid/os/Parcel;->readInt()I

    move-result v2

    if-eqz v2, :cond_5

    .line 109
    sget-object v2, Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;->CREATOR:Landroid/os/Parcelable$Creator;

    invoke-interface {v2, p2}, Landroid/os/Parcelable$Creator;->createFromParcel(Landroid/os/Parcel;)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;

    .local v2, "_arg0":Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;
    goto :goto_2

    .line 112
    .end local v2    # "_arg0":Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;
    :cond_5
    const/4 v2, 0x0

    .line 114
    .restart local v2    # "_arg0":Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;
    :goto_2
    invoke-virtual {p0, v2}, Landroid/net/IConnectivityDiagnosticsCallback$Stub;->onDataStallSuspected(Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;)V

    .line 115
    return v1

    .line 93
    .end local v2    # "_arg0":Landroid/net/ConnectivityDiagnosticsManager$DataStallReport;
    :cond_6
    invoke-virtual {p2, v0}, Landroid/os/Parcel;->enforceInterface(Ljava/lang/String;)V

    .line 95
    invoke-virtual {p2}, Landroid/os/Parcel;->readInt()I

    move-result v2

    if-eqz v2, :cond_7

    .line 96
    sget-object v2, Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;->CREATOR:Landroid/os/Parcelable$Creator;

    invoke-interface {v2, p2}, Landroid/os/Parcelable$Creator;->createFromParcel(Landroid/os/Parcel;)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;

    .local v2, "_arg0":Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;
    goto :goto_3

    .line 99
    .end local v2    # "_arg0":Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;
    :cond_7
    const/4 v2, 0x0

    .line 101
    .restart local v2    # "_arg0":Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;
    :goto_3
    invoke-virtual {p0, v2}, Landroid/net/IConnectivityDiagnosticsCallback$Stub;->onConnectivityReportAvailable(Landroid/net/ConnectivityDiagnosticsManager$ConnectivityReport;)V

    .line 102
    return v1
.end method
